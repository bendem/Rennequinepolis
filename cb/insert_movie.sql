...
begin
    ...
    if exist = 1 then
        update movies set movie_copies = movie_copies + round((SYS.DBMS_RANDOM.NORMAL*2)+5) where movie_id = v_movie.id;
    else
        raw_data := v_movie;
        ...
        if raw_data.status is not null then
            ...
            select null into movie_rec.movie_status_id from dual;
        end if;
        ...
        if raw_data.certification is not null then
            ...
            select null into movie_rec.movie_certification_id from dual;
        end if;
