    ...
    begin
        ...
        (
            select XMLElement("movie",
                ...
                (select XMLAgg(XMLElement("actor",
                    XMLForest(
                        person_id as "id",
                        person_name as "name",
                        image as "picture",
                        character_name as "character_name")))
                from characters natural join people left join images on (person_profile_id = image_id) where movie_id = p_id),
                (select XMLAgg(XMLElement("director",
                    XMLForest(
                        person_id as "id",
                        person_name as "name",
                        image as "picture")))
                from movies_directors natural join people left join images on (person_profile_id = image_id) where movie_id = p_id),
                (select XMLAgg(XMLElement("production_company", production_company_name))
                from production_companies natural join movies_production_companies where movie_id = p_id),
                (select XMLAgg(XMLElement("production_country", production_country_name))
                from production_countries natural join movies_production_countries where movie_id = p_id),
                (select XMLAgg(XMLElement("genre", genre_name))
                from genres natural join movies_genres where movie_id = p_id),
                (select XMLAgg(XMLElement("review",
                    XMLForest(
                        rating as "rating",
                        creation_date as "creation_date",
                        content as "content")))
                from reviews where movie_id = p_id and backup_flag <> 2),
                (select XMLAgg(XMLElement("spoken_language", spoken_language_name))
                from spoken_languages natural join movies_spoken_languages where movie_id = p_id)
            ) from movies m left join certifications on (m.movie_certification_id = certification_id)
