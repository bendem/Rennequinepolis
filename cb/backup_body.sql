    ...
    begin
        ...
        merge into images@link.backup there using (
            select * from images where image_id in (
                select movie_poster_id from movies where movie_id in (select movie_id bulk from movies where backup_flag = 0)
                union
                select person_profile_id from people where person_id in (
                    select person_id from movies_directors where movie_id in (select movie_id bulk from movies where backup_flag = 0)
                    union
                    select person_id from characters where movie_id in (select movie_id bulk from movies where backup_flag = 0)
                )
            )
        ...
        merge into genres@link.backup there using (
            select * from genres where genre_id in (
                select genre_id from movies_genres where movie_id in (select movie_id bulk from movies where backup_flag = 0)
            )
        ...
        merge into certifications@link.backup there using (
            select * from certifications where certification_id in (
                select certification_id from movies where movie_id in (select movie_id bulk from movies where backup_flag = 0)
            )
        ...
        merge into statuses@link.backup there using (
            select * from statuses where status_id in (
                select status_id from movies where movie_id in (select movie_id bulk from movies where backup_flag = 0)
            )
        ...
        merge into spoken_languages@link.backup there using (
            select * from spoken_languages where spoken_language_id in (
                select spoken_language_id from movies_spoken_languages where movie_id in (select movie_id bulk from movies where backup_flag = 0)
            )
        ...
        merge into production_countries@link.backup there using (
            select * from production_countries where production_country_id in (
                select production_country_id from movies_production_countries where movie_id in (select movie_id bulk from movies where backup_flag = 0)
            )
        ...
        merge into production_companies@link.backup there using (
            select * from production_companies where production_company_id in (
                select production_company_id from movies_production_companies where movie_id in (select movie_id bulk from movies where backup_flag = 0)
            )
        ...
        merge into movies_production_countries@link.backup there using (
            select * from movies_production_countries where movie_id in (select movie_id bulk from movies where backup_flag = 0)
        ...
        merge into movies_production_companies@link.backup there using (
            select * from movies_production_companies where movie_id in (select movie_id bulk from movies where backup_flag = 0)
        ...
        merge into movies_directors@link.backup there using (
            select * from movies_directors where movie_id in (select movie_id bulk from movies where backup_flag = 0)
        ...
        merge into movies_genres@link.backup there using (
            select * from movies_genres where movie_id in (select movie_id bulk from movies where backup_flag = 0)
        ...
        merge into characters@link.backup there using (
            select * from characters where movie_id in (select movie_id bulk from movies where backup_flag = 0)
        ...
        merge into copies@link.backup there using (
            select * from copies where movie_id in (select movie_id bulk from movies where backup_flag = 0)
        ) here on (there.movie_id = here.movie_id)
        when not matched then
            insert values (
                here.movie_id,
